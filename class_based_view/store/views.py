...
class IndexView(View):
  ...
  def get(self, request, *args, **kwargs):
    ...
    return render(request, 'index.html', context={
        'book_form':book_form,
      }) #index.htmlを画面上に表示することができる。そのためにsettings.pyに記載
    
  def post(self, request, *args, **kwargs): #postmethodを定義する。
...
class BookListView(ListView):
  model = Books #一覧表示するモデル
  template_name = 'book_list.html' #表示するテンプレートの名前
...
class BookFormView(FormView):
  ...
  def form_valid(self, form): #POSTリクエストであればバリデーションチェックして保存してね。
    if form.is_valid():
      form.save()
